        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="min-w-64 flex-1">
            ...
            <p className="-mt-1 text-sm leading-relaxed text-ink-600 dark:text-ink-300">
              {L({
                uk: 'Консультант DEMAX побудований на тій самій технології, що й наш власний продукт Polylog: багатомовний AI-асистент з RAG, гібридним пошуком і перевіркою, що відповідь спирається на джерела, а не вигадана. Ми не винаходимо з нуля — переносимо перевірене в продакшені. Веде проєкт Віталій Кіндракевич, засновник DevMotion Studio: 10+ років у веб-розробці, 40+ проєктів у продакшені, щоденна робота з AI із 2024 року.',
                ru: 'Консультант DEMAX построен на той же технологии, что и наш собственный продукт Polylog: мультиязычный AI-ассистент с RAG, гибридным поиском и проверкой, что ответ опирается на источники, а не выдуман. Мы не изобретаем с нуля — переносим проверенное в продакшене. Ведёт проект Виталий Киндракевич, основатель DevMotion Studio: 10+ лет в веб-разработке, 40+ проектов в продакшене, ежедневная работа с AI с 2024 года.',
                en: 'The DEMAX consultant is built on the same technology as our own product Polylog: a multilingual AI assistant with RAG, hybrid retrieval and a groundedness check that blocks answers not supported by sources. We are not inventing from scratch — we are porting what already runs in production. Led by Vitalii Kindrakevych, founder of DevMotion Studio: 10+ years in web development, 40+ production projects, working with AI daily since 2024.',
              })}
            </p>
          </div>
        ...
        <Card className="animate-rise-2 lg:col-span-3">
          ...
          <ul className="space-y-3 text-sm leading-relaxed text-ink-600 dark:text-ink-300">
            {([
              ...
              { uk: 'Команда без накладних витрат: два senior-інженери, пряма комунікація із замовником, нульова бюрократія — за весь шлях від архітектури до продакшену відповідає одна людина, а не ланцюжок передачі задач.', ru: 'Команда без накладных расходов: два senior-инженера, прямая коммуникация с заказчиком, нулевая бюрократия — за весь путь от архитектуры до продакшена отвечает один человек, а не цепочка передачи задач.', en: 'A team with zero overhead: two senior engineers, direct client communication, no bureaucracy — one person is accountable for the whole path from architecture to production, not a chain of handoffs.' },
            ] as Bi[]).map((b, i) => (
