  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-5">
        ...
        {/* measured evaluation of the retrieval quality */}
      <Card className="animate-rise-2 mb-4 lg:col-span-3">
        <div className="mb-3 flex items-center gap-3">
          <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-copper-600/12 text-copper-700 dark:text-copper-300"><Gauge size={19} /></span>
          <div>
            <SectionTitle>{L({ uk: 'Ми міряємо якість пошуку, а не вгадуємо', ru: 'Мы измеряем качество поиска, а не угадываем', en: 'We measure retrieval quality instead of guessing' })}</SectionTitle>
            <p className="-mt-2 text-xs text-ink-500 dark:text-ink-400">
              {L({
                uk: '12 реальних питань · дві моделі ембедінгів · сліпий LLM-суддя, який не знав, яка модель що знайшла',
                ru: '12 реальных вопросов · две модели эмбеддингов · слепой LLM-судья, не знавший, какая модель что нашла',
                en: '12 real questions · two embedding models · a blind LLM judge that did not know which model retrieved what',
              })}
            </p>
          </div>
        </div>
        ...
        <div className="mb-4 overflow-x-auto">
          <table className="w-full min-w-96 text-sm">
            <thead>
              <tr className="border-b border-ink-200/70 text-left text-xs uppercase tracking-wide text-ink-500 dark:border-ink-700 dark:text-ink-400">
                <th className="py-2 pr-3 font-medium">{L({ uk: 'Конфігурація', ru: 'Конфигурация', en: 'Configuration' })}</th>
                <th className="py-2 pr-3 text-right font-medium tabular-nums">P@5</th>
                <th className="py-2 pr-3 text-right font-medium tabular-nums">MRR</th>
                <th className="py-2 text-right font-medium">{L({ uk: 'Питань із влучанням', ru: 'Вопросов с попаданием', en: 'Questions with a hit' })}</th>
              </tr>
            </thead>
            <tbody className="text-ink-700 dark:text-ink-200">
              {([
                ['лише вектор · MiniLM (384)', 'только вектор · MiniLM (384)', 'vector only · MiniLM (384)', '0.250', '0.417', '7/12', false],
                ['лише вектор · bge-m3 (1024)', 'только вектор · bge-m3 (1024)', 'vector only · bge-m3 (1024)', '0.150', '0.278', '6/12', false],
                ['гібрид + RRF · MiniLM', 'гибрид + RRF · MiniLM', 'hybrid + RRF · MiniLM', '0.200', '0.357', '8/12', true],
                ['гібрид + RRF · bge-m3', 'гибрид + RRF · bge-m3', 'hybrid + RRF · bge-m3', '0.183', '0.315', '7/12', false],
              ] as [string, string, string, string, string, string, boolean][]).map((r) => (
                <tr key={r[2]} className={`border-b border-ink-100 last:border-0 dark:border-ink-800 ${r[6] ? 'bg-copper-600/8 font-semibold' : ''}`}>
                  <td className="py-2 pr-3">{L({ uk: r[0], ru: r[1], en: r[2] })}</td>
                  <td className="py-2 pr-3 text-right tabular-nums">{r[3]}</td>
                  <td className="py-2 pr-3 text-right tabular-nums">{r[4]}</td>
                  <td className="py-2 text-right tabular-nums">{r[5]}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <ul className="space-y-3 text-sm leading-relaxed text-ink-600 dark:text-ink-300">
          {([
            {
              uk: 'Переходити на важчу bge-m3 підстав немає: якості вона не додала, а індекс потребує у 2,7 раза більше місця й окремого сервісу. Різниця між моделями на 12 питаннях лежить у межах шуму.',
              ru: 'Переходить на более тяжёлую bge-m3 оснований нет: качества она не добавила, а индекс требует в 2,7 раза больше места и отдельного сервиса. Разница между моделями на 12 вопросах лежит в пределах шума.',
              en: 'There is no case for switching to the heavier bge-m3: it added no quality, while its index needs 2.7× more space and a separate service. On 12 questions the gap between models is within noise.',
            },
            {
              uk: 'Головний висновок інший: вузьке місце — не модель, а наповнення бази знань. Обидві моделі провалюються на тих самих питаннях, бо релевантного змісту в базі просто немає — сторінка «Розклад семінарів» не містить переліку семінарів, а сторінки товарів дають по одному короткому фрагменту.',
              ru: 'Главный вывод другой: узкое место — не модель, а наполнение базы знаний. Обе модели проваливаются на одних и тех же вопросах, потому что релевантного содержания в базе просто нет — страница «Расписание семинаров» не содержит перечня семинаров, а страницы товаров дают по одному короткому фрагменту.',
              en: 'The real finding is different: the bottleneck is the knowledge base, not the model. Both models fail on the same questions because the content simply is not there — the seminars page carries no seminar list, and product pages yield a single short chunk each.',
            },
            {
              uk: 'Під час вимірювання знайшли й виправили дефект: лексична гілка пошуку вимагала збігу всіх слів питання одразу і мовчала на 11 питаннях із 12 — тобто гібридний пошук фактично працював як звичайний векторний. Тепер вона спрацьовує на 10 із 12.',
              ru: 'В ходе измерения нашли и исправили дефект: лексическая ветвь поиска требовала совпадения всех слов вопроса сразу и молчала на 11 вопросах из 12 — то есть гибридный поиск фактически работал как обычный векторный. Теперь она срабатывает на 10 из 12.',
              en: 'The measurement also exposed a defect we fixed: the lexical branch required every word of the question to match at once and stayed silent on 11 of 12 questions — so hybrid search was effectively vector-only. It now fires on 10 of 12.',
            },
            {
              uk: 'Що це означає для проєкту: перші тижні варто витратити не на добір моделі, а на матеріали — PIF, презентації семінарів, картки товарів зі складом. Саме вони дають приріст якості, який видно клієнту.',
              ru: 'Что это значит для проекта: первые недели стоит потратить не на подбор модели, а на материалы — PIF, презентации семинаров, карточки товаров с составом. Именно они дают прирост качества, заметный клиенту.',
              en: 'What this means for the project: the first weeks should go into materials — PIF documents, seminar decks, product cards with ingredients — not into model selection. That is where the quality gain the client can see comes from.',
            },
          ] as Bi[]).map((b, i) => (
